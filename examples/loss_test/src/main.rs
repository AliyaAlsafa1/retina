...
// Optional: if a packet matches one of our preinstalled tuples, log it.
// This should basically never happen if you keep the IP ranges disjoint
// from pktgen’s traffic.
#[filter("tcp")]
fn tcp_cb(tuple: &FiveTuple, _core: &CoreId) {
    let t = TARGET_SET.lock().unwrap();
    if t.contains(tuple) {
        eprintln!("[warn] observed TCP for a preinstalled drop tuple: {:?}", tuple);
    }
}

// Not used
#[filter("tls")]
fn tls_cb(_tls: &TlsHandshake, _rec: &ConnRecord) {}
...
fn main() {
    ...
    // 4) Spawn the installer thread (unchanged)
    let installer = std::thread::spawn(move || {
        std::thread::sleep(std::time::Duration::from_millis( args.warmup_ms ));
        let ports = {
            let guard = PORT_IDS.read().unwrap();
            guard.as_ref().cloned().expect("PORT_IDS not set")
        };
    });
    /*
    ...
    let installer = std::thread::spawn(move || {
        ...
        if uninstall_on_exit {
            ctrlc::set_handler(|| {
                eprintln!("\n[loss_test] signal received — uninstalling {} flows…",
                ...
                eprintln!("[loss_test] uninstall complete. exiting.");
                std::process::exit(0);
...
*/
    ...
    // 5) Run the runtime on the main thread (blocks)
    runtime.run();
